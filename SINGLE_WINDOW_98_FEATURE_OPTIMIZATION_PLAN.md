...
更新时间：2026-07-16
适用版本：`stage2_interpretable_v10`，126 项受治理候选
...
- 100 Hz PPG/ACC 固定保留索引 `0,4,8,...`（`x[::4]`）降为 25 Hz；原生 25 Hz 不处理。
- 每条数据读取并完成窗口排序后保留全部合法窗口，不自动裁剪首尾窗口，也不提供额外窗口跳过参数。
- s03 不设置样本处理超时；所有样本均会被尝试，提取异常整体报错，不允许静默漏数。
- XGBoost 始终使用全量合法窗口，不经过 IR DC/ACDC 阈值过滤或结果融合。
- 模型特征池不使用 IR 派生特征；最终判决和全部统计均来自 XGBoost。
- 特征池为 `stage2_interpretable_v10`/126，包含 `mode`、商用八特征映射、三固定位置绿光区及鲁棒空间候选。
...
不能修改特征名、排序、公式、版本、风险字段或 ranking SHA。保存后使用以下命令跳过已完成的
s01、s03 和 s04，并从 s05 恢复训练：
...
特征脚本自包含人工选择后的顺序、fill/clip、阈值和特征引擎，不依赖项目源码。
NumPy 和 XGBoost 属于 Python 运行环境依赖。其他 catalog、cookbook 和 golden-vector 文件用于审计、
固件移植及数值对齐，不是 Python 单窗推理的必需项目文件。

## 验证命令

```powershell
python -m py_compile s03_extract_feature_pool.py s05_train_final_model.py s06_deploy_eval.py s07_postprocess_optimize.py s08_run_pipeline.py stage2_feature_catalog.py commercial_liveness_features.py direct_feature_selection.py manual_feature_selection.py model_search_limits.py pipeline_acceptance.py scientific_figures.py
