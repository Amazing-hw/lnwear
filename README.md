...
```text
s01 -> s02 -> s03 -> s04 -> s04_search -> s05 -> s06_eval -> s06_xpt -> s06_feat -> s06_plot -> s06_cb
```
...
当 `--stop_after s10_audit` 被指定时，`s08` 会自动打开 `--run_generalization_audit`。
...
- `--stop_after s10_audit` 会按需打开 `--run_generalization_audit`。
