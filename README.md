...
The current recommended flow follows the simpler `new_new` project style:
...
- run `s01 -> s02 -> s03 -> s04 -> s04_search -> s05 -> s06_eval -> deploy exports`
- keep `model_search` enabled
- optimize the window model for accuracy by default
- do not run hard-negative mining
- do not run staged E2E child pipelines
- do not run postprocess search unless explicitly requested

Run postprocess search explicitly with:

```bash
python s08_run_pipeline.py --dataset_dir dataset --artifact_dir artifacts --with_postprocess
```

The old `--hard_negative_optimize` and `--staged_e2e_optimize` shortcuts are intentionally rejected by `s08_run_pipeline.py`. The lower-level `s05 --mine_hard_negatives` code is retained only for manual experiments and compatibility tests; it is not part of the recommended pipeline.
...
`--accuracy_first_optimize` 会把阈值目标切到 `accuracy`，聚焦窗口准确率，不自动导出 NPZ、不跑 s07 后处理搜参、不跑 s10 审计，也不自动提高 hard negative 权重。旧的 `--hard_negative_optimize` 快捷入口已经从推荐 `s08` 流程移除；如需后处理搜参，使用 `--with_postprocess`。
...
这条命令会在默认准确率优先主流程基础上额外导出 valid/test 窗口缓存，并运行 s07 后处理搜参。它不会触发 train-only OOF hard negative 挖掘，也不会把窗口模型阶段自动切到 `precision_constrained`。
