...
def _render_deployment_feature_extractor(selected_features, fill_values, clip_bounds, formulas,
                                         ...
                                         window_sec=5.0,
                                         scripts_dir=None):
    """Render a source-backed Python extractor for parity checks.

    s03_extract_feature_pool.py is the single source of truth for feature formulas.
    This generated helper preserves selected feature order, fill, clip, and threshold
    metadata while delegating feature computation back to s03.
    """
    ...
    scripts_dir_json = json.dumps(str(scripts_dir or SCRIPTS_DIR), ensure_ascii=False)

    return f'''# -*- coding: utf-8 -*-
"""Auto-generated source-backed deployment feature extractor.

This Python helper is for parity checks and golden-vector generation. Feature
formulas live in s03_extract_feature_pool.py; do not edit this file to create a
second feature implementation. Port selected formulas from
FEATURE_FORMULAS/deploy_selected_feature_formulas.json to C for firmware.
"""
...
S03_SOURCE_DIR = Path({scripts_dir_json})
EPS = 1e-12


if str(S03_SOURCE_DIR) not in sys.path:
    sys.path.insert(0, str(S03_SOURCE_DIR))
