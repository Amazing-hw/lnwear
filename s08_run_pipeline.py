...
已知特征清单时可直接做选择式提取并训练，跳过 s04：
    python s08_run_pipeline.py --dataset_dir dataset --artifact_dir artifacts_direct --direct_features "mode,GTOP2_CORR,ACC_MAG_MEAN"
    python s08_run_pipeline.py --dataset_dir dataset --artifact_dir artifacts_direct --direct_feature_file my_features.csv
...
legacy s06 状态机优化、NPZ 缓存导出和 s07 后处理搜参很耗时，默认不跑；需要时显式运行：
    python s08_run_pipeline.py --artifact_dir artifacts --skip s01,s03,s04 --optimize
...
XGBoost no-IR policy:
    IR-derived features are excluded. Feature selection,
...
from direct_feature_selection import (
    ...
    parse_direct_features,
    write_direct_feature_csv,
)
...
def _load_eval_details(artifact_dir, split="test", method="prob_mean"):
...
def export_deploy_cookbook(artifact_dir):
    ...
    cookbook = {
        ...
        "A_preprocessing": {
            "_note": "对 ambient/g1/g2/g3 执行以下管线，产出 contact_raw、pulse 和 dc；XGBoost 不使用 IR 派生特征",
        ...
        # ---- Section D: optional postprocess ----
        "D_optional_postprocess": {
...
def generate_eval_csv(artifact_dir, split="test", method="prob_mean"):
    ...
    # ---- CSV 1: XGBoost 单窗预测 ----
    # 与 compute_window_model_metrics 一致：统计全部合法窗口。
    rows_xgb = []
    for d in details:
    ...
    # ---- CSV 3: 样本级最终预测 ----
    # 与 compute_sample_metrics 一致：pred 是 XGBoost-only 样本输出。
    rows_sample = []
    for d in details:
        name = d.get("sample_name", "")
        ...
        rows_sample.append((name, target, pred, is_correct, n_win, fallback))

    csv3 = _os.path.join(artifact_dir, "per_sample_final_prediction.csv")
    with open(csv3, "w", encoding="utf-8") as f:
        f.write("sample_name,target,pred,is_correct,total_windows,is_fallback\n")
...
def plot_error_samples(artifact_dir, split="test", method="prob_mean",
    ...
    for d in errors:
        ...
        ax.set_title(f"{d['sample_name']}  target={target}  pred={pred}  "
                     f"fallback={d.get('fallback',False)}",
                     fontsize=10)
        if n_win > 0:
...
""")
    ...
    p.add_argument(
        ...
        default="manual",
        help="manual 先做 s04 排序；auto 无人值守选特征；direct 按给定清单选择式提取并训练",
    )
    p.add_argument(
        "--manual_feature_file",
        default=None,
    ...
    direct_group = p.add_mutually_exclusive_group()
    direct_group.add_argument(
        "--direct_features",
        default=None,
        help="comma-separated governed feature names; exact order is used for extraction, training, and deployment",
    )
    direct_group.add_argument(
        "--direct_feature_file",
        default=None,
        help="CSV with exactly one 'feature' column; row order is the model feature order",
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "manual/direct 的实际特征数完全由 CSV 决定"
        ),
    )
    ...
    p.add_argument("--mine_hard_negatives", action=argparse.BooleanOptionalAction, default=None,
                   help="train an OOF train-only hard-negative candidate; defaults on for manual/direct fixed-feature training")
    ...
    p.add_argument("--model_search_feature_counts", type=str, default="8,10,12,15,18",
                   help="仅 auto 模式使用；manual/direct 严格使用 CSV，不搜索特征数量")
    ...
    p.add_argument("--model_search_cache", action=argparse.BooleanOptionalAction, default=True,
                   help="reuse versioned Stage-A/Stage-B search metrics under artifact_dir")
    p.add_argument(
    ...
    direct_feature_file = None
    direct_selected_features = None
    if args.direct_features is not None or args.direct_feature_file is not None:
        if args.commercial_only:
            p.error("--commercial_only conflicts with direct feature selection")
        if "--feature_selection_mode" in _raw_argv and args.feature_selection_mode != "direct":
            p.error("direct feature arguments require --feature_selection_mode direct (or omit the mode)")
        args.feature_selection_mode = "direct"
        try:
            direct_selected_features = (
                parse_direct_features(args.direct_features)
                if args.direct_features is not None
                else load_direct_feature_csv(args.direct_feature_file)
            )
        except ValueError as exc:
            p.error(str(exc))
        direct_feature_file = os.path.abspath(os.path.join(
            args.artifact_dir, "direct_feature_selection.csv"
        ))
        write_direct_feature_csv(direct_feature_file, direct_selected_features)
        args.max_features = len(direct_selected_features)
        print(
            f"[direct] frozen {len(direct_selected_features)} features -> "
            f"{direct_feature_file}"
        )
    elif args.feature_selection_mode == "direct":
        p.error("--feature_selection_mode direct requires --direct_features or --direct_feature_file")
    if direct_feature_file and (args.auto_optimize_e2e or args.full_optimize):
        p.error(
            "direct feature selection conflicts with --auto_optimize_e2e/--full_optimize "
            "because those modes search feature subsets; use --with_postprocess if needed"
        )
    ...
    if args.feature_selection_mode == "direct":
        skip_set.update({"s04", "s04_search", "s04_embed"})
        if stop_after in {"s04", "s04_search", "s04_embed"}:
            stop_after = "s05"
        direct_budgets = {
            "fast": (60, 6),
            "balanced": (80, 8),
        ...
        direct_max_candidates, direct_stage2_top_k = direct_budgets[args.runtime_profile]
        if not _arg_was_provided(_raw_argv, "--model_search_max_candidates"):
            args.model_search_max_candidates = direct_max_candidates
        if not _arg_was_provided(_raw_argv, "--model_search_stage2_top_k"):
            args.model_search_stage2_top_k = direct_stage2_top_k
        print(
            "[direct] s04 ranking/subset/embedding steps are skipped; exact CSV "
            "feature names/order/count are frozen."
        )
        print(
            "[direct] model-search budget: "
            f"stage_a={args.model_search_max_candidates}, "
            f"stage_b={args.model_search_stage2_top_k}"
        )
    elif args.feature_selection_mode == "manual" and not manual_resume:
        if args.commercial_only:
        ...
        if not _arg_was_provided(_raw_argv, "--model_search_max_candidates"):
            args.model_search_max_candidates = manual_max_candidates
        if not _arg_was_provided(_raw_argv, "--model_search_stage2_top_k"):
            args.model_search_stage2_top_k = manual_stage2_top_k
        print(
            "[manual] exact CSV feature names/order/count are frozen; "
            "feature-count search is disabled."
        )
        print(
            "[manual] model-search budget: "
            f"stage_a={args.model_search_max_candidates}, "
            f"stage_b={args.model_search_stage2_top_k}"
        )
    ...
    model_search_cache_flag = (
        "--model_search_cache" if args.model_search_cache else "--no-model_search_cache"
    )
    mine_hard_negatives = (
        args.feature_selection_mode in {"manual", "direct"}
    ...
    if "s03" not in skip_set:
        ...
        _direct_extract_flag = (
            f' --selected_feature_file "{direct_feature_file}"'
            if args.feature_selection_mode == "direct" else ""
        )
        commands["s03"] = (
    ...
    if "s05" not in skip_set:
        ...
        if args.feature_selection_mode == "manual":
            ...
            selection_args += f'--direct_feature_file "{direct_feature_file}" '
        local_swap_flag = (
            "--no-feature_search_local_swap"
            if args.feature_selection_mode in {"manual", "direct"}
    ...
    if args.feature_selection_mode == "direct":
        print(f"  direct feature CSV: {direct_feature_file}")
        print(f"  direct feature count: {len(direct_selected_features)}")
    elif manual_resume:
